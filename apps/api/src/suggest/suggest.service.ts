  ...
  async suggest(request: SuggestRequestDto): Promise<SuggestResponseDto> {
    ...
    try {
      ...
      try {
        ...
        lexMap = new Map<string, number>(lex.map((it: any) => [String(it.item.item), Number(it.score) || 0]));
      } catch (e) {
      ...
      const maxBm = rows.length > 0 ? Math.max(...rows.map((r: any) => Number(r.bm25) || 0)) : 0;
      if (maxBm > 0) {
        rows = rows.map((r: any) => ({ ...r, bm25n: Math.max(0, Math.min(1, (Number(r.bm25) || 0) / maxBm)) }));
      } else {
      ...
      const withRules = ranked.map((c) => {
        ...
        const bm25 = Math.max(0, Math.min(1, Number((c as any).score_breakdown?.bm25 ?? 0)));
        const scoreRaw = Number((c as any).score_breakdown?.score_raw ?? 0);
        const alpha = Math.max(0, Math.min(1, Number(process.env.RANKER_ALPHA ?? 0.7)));
        const beta = Math.max(0, Math.min(1, Number(process.env.RANKER_BETA ?? 0.3)));
        let featureRaw = beta > 0 ? (scoreRaw - alpha * bm25) / beta : 0;
        featureRaw = Math.max(0, Math.min(1, featureRaw));
        const wr = Number.isFinite(Number(process.env.SUGGEST_RAG_WEIGHT)) ? Number(process.env.SUGGEST_RAG_WEIGHT) : 0.7;
        const wf = Number.isFinite(Number(process.env.SUGGEST_FEAT_WEIGHT)) ? Number(process.env.SUGGEST_FEAT_WEIGHT) : 0.1;
        const wl = Number.isFinite(Number(process.env.SUGGEST_LEX_WEIGHT)) ? Number(process.env.SUGGEST_LEX_WEIGHT) : 0.2;
        ...
        const lex = Math.max(0, Math.min(1, Number((original as any)?._lex ?? 0)));
        let baseSim = Math.max(0, Math.min(1, (wRag * bm25 + wFeat * featureRaw + wLex * lex) / norm));
        // Consistency boost if both RAG and Lexical hit
        const agreeBoost = Number.isFinite(Number(process.env.SUGGEST_AGREEMENT_BOOST)) ? Number(process.env.SUGGEST_AGREEMENT_BOOST) : 1.06;
        if (bm25 > 0 && lex > 0) baseSim = Math.max(0, Math.min(1, baseSim * agreeBoost));
        // Power stretch to amplify separation (gamma > 1 raises top, lowers tail)
        const gamma = Number.isFinite(Number(process.env.SUGGEST_GAMMA)) ? Number(process.env.SUGGEST_GAMMA) : 1.35;
        if (gamma && gamma !== 1) baseSim = Math.max(0, Math.min(1, Math.pow(baseSim, gamma)));
        ...
        if (threshold && typeof signalsInternal.duration === 'number' && signalsInternal.duration > threshold) {
          ...
          const marginCap = Number.isFinite(Number(process.env.SUGGEST_MARGIN_MAX)) ? Number(process.env.SUGGEST_MARGIN_MAX) : 0.25;
          marginBonus = Math.max(0, Math.min(marginCap, (delta / 60) * marginCap));
        }
        ...
        const warnDecay = Number.isFinite(Number(process.env.SUGGEST_WARN_DECAY)) ? Number(process.env.SUGGEST_WARN_DECAY) : 0.09;
        const warnMax  = Number.isFinite(Number(process.env.SUGGEST_WARN_MAX))   ? Number(process.env.SUGGEST_WARN_MAX)   : 0.35;
        const sRuleSoft2 = softTotal > 0 ? Math.max(0, 1 - Math.min(warnMax, warnCount * warnDecay)) : 1;
        ...
        const penaltiesWeight = Number.isFinite(Number(process.env.SUGGEST_PENALTY_WEIGHT)) ? Number(process.env.SUGGEST_PENALTY_WEIGHT) : 0.8;
        score = score * (0.75 + 0.25 * sRuleSoft2) * (0.75 + 0.25 * evidenceSuff2) * (1 - Math.max(0, Math.min(1, (penalties ?? 0) * penaltiesWeight)));
        // sharper sigmoid and shifted center for larger separation
        const k = Number.isFinite(Number(process.env.SUGGEST_SIGMOID_K)) ? Number(process.env.SUGGEST_SIGMOID_K) : 4.5;
        const center = Number.isFinite(Number(process.env.SUGGEST_SIGMOID_CENTER)) ? Number(process.env.SUGGEST_SIGMOID_CENTER) : 0.42;
